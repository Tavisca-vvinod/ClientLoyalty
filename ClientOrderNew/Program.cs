using ClientOrderNew.ServiceReference1;
using System;
...
namespace ClientOrderNew
{
    class Program
    {
        ...
        public static object DataContractDeSerialize(string data, Type type)
        {
            try
            {
                using (MemoryStream memoryStream = new MemoryStream(Encoding.UTF8.GetBytes(data)))
                {
                    ...
                    return dcs.ReadObject(memoryStream);
                }
            }
            catch (Exception ex)
            {
